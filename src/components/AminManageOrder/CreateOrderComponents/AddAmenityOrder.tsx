...
const AddAmenityOrder: React.FC<AddAmenityOrderProps> = ({ bookingData, setBookingData }) => {
  ...
  const handleIncrement = () => {
    if (!detailAmenity) {
      ...
      if (detailAmenity.type === 'Office') {
        const room1 = bookingData.selectedRooms.filter((room1: RoomContextType) => room1.name === room)[0]
        const preAmennity = room1?.amenities.filter((item: Amenity) => item.name === detailAmenity.name)
  ...
  return (
    <Box sx={{ paddingRight: '12px', marginTop: '10px', bgcolor: 'white', padding: 2, borderRadius: '5px' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', padding: '20px 20px 0px 0px', gap: '24px' }}>
        <FormControl fullWidth>
          ...
          >
            {bookingData?.selectedRooms.map((room: RoomContextType, index: number) => (
